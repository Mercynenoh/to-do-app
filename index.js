...
displayTasks=()=>{
  ...
  const taskContainers = getAllTasks().map((task, index)=>`
    <div class='task'>
      <h2>${task.title}</h2>
      <h4>${task.desc}</h4>
      <p>${task.duedate}</p>
...
completeTask=(index)=> {
  ...
  inputs.checked = true;
   if (inputs.checked===true){
    const completed=taskArr.map((task, index)=>`
    <div class='task'>
      <h2>${task.title}</h2>
      <h4>${task.desc}</h4>
      <p>${task.duedate}</p>
...
// let title,desc,duedate,diff=''
  completedTasks.map(function (item, i){
    ...
    p2.textContent=`Completed in ${item.diff} days`
    // alert("Congratulations you submitted on time")
    // }
    // else{
    //   alert("You are late")
    // }
